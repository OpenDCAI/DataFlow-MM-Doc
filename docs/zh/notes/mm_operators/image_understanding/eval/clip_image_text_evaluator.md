...
本算子基于 CLIP 模型计算图像与文本的对齐分数（范围 [0,1]）。你可以使用本地权重或 HuggingFace 上的模型名称（如`openai/clip-vit-base-patch32`）。
...
storage = FileStorage(
    first_entry_file_name="data/clip_input.jsonl",  # 至少包含 image_path 与 text 两列
...
evaluator = CLIPEvaluator(
    model_name="openai/clip-vit-base-patch32",  # 可替换为你的本地路径
...
evaluator.run(
    ...
    text_key="text",          # 输入列：文本描述
    output_key="clip_score"   # 输出列：分数字段，范围 [0,1]
)
```
...
生成的 `jsonl` 文件中会新增 `clip_score` 字段，例如：
