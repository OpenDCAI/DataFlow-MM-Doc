...
## 语音识别与清洗流水线
该流水线用于对输入音频完成以下处理链路：
- ASR 语音识别：使用 `Whisper`（本地 `vLLM Serving` 或 `OpenAI API Serving`）生成转写文本；
- 文本归一化清洗：对转写结果做规范化（如英文大小写/符号处理等），可选去标点；
- CTC 强制对齐评估/过滤：使用 `CTC forced aligner` 对音频与转写文本进行对齐，输出逐词/片段级别的时间戳与对齐置信度，便于质量评估或过滤。
...
#     api_url="http://127.0.0.1:8091/v1",  # 本地部署的模型api_url
#     max_workers=3,
...
## 第八步：初始化CTC强制对齐评估或过滤算子
```python
# 如果使用过滤算子, 选择CTCForcedAlignmentFilter
